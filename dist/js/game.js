...
class CharacterSkillSystem {
  ...
  update() {
    ...
    // Update skill bar
    for (let i = 1; i <= this.scene.skillBar.length; i++) {
      const part = this.scene.skillBar[i - 1];
      
      part.visible = i <= this.character.skillLevel;
      part.frame = this.getSkillsUsed() < i ? 0 : 1;
    }
  }
...
class Test {
  create() {
    new BackgroundGradient();
    new FuturisticLines();    new TextInput("", 6);
  }
  update() {
    gamepad.update();
  }
...
class TextInput extends Phaser.Sprite {
  constructor(text = "", maxLength = 6) {
    super(game, 96, 28);
    this.anchor.x = 0.5;
    
    this.characterSet =  "ABCDEFGHIJKLMNOPQRSTUVWXYZ.,:!¡?¿h+-×*()[]/\\0123456789_'\"`•<>=% ";
    
    this.window = new Window(0, 0, maxLength + 1, 2, "1", this);
    ...
    this.cursor = game.add.graphics(3, 6);
    this.cursor.beginFill(this.window.fontTint, 1);
    this.cursor.drawRect(0, 0, 2, 4);
    this.cursor.endFill();
    
    this.textLayer = new Text(3, 6, text);
    this.window.addChild(this.textLayer);
    
    game.time.events.loop(350, () => this.cursor.visible = !this.cursor.visible);
    
    this.window.addChild(this.cursor);
    
    game.add.existing(this);
  },
  getCharacterToInsert() {
    return this.characterSet[this.lastCharacterIndex];
  }
  update() {
    const isAtMaxLength = this.stackedText.length < this.maxLength;
    
    this.textLayer.text = isAtMaxLength ? this.stackedText : this.stackedText + this.getCharacterToInsert();
    
    this.cursor.x = (isAtMaxLength ? this.stackedText.length : this.stackedText.length + 1) * 8 + 3;
    
    if (!isAtMaxLength) {
      let newIndex = this.lastCharacterIndex;
      ...
      this.lastCharacterIndex = newIndex;
    }
  }
}
...
class Play {
  ...
  createHud() {
    ...
    for (let i = 0, x = 6; i < 5; i++, x += 3) {
      const part = game.add.sprite(x, 15, 'ui_skill_bar', 0);
      this.hud.addChild(part);
      this.skillBar.push(part);
    }
